...
def get_all_sports_as_list():
    ...
    request_url = 'https://api.the-odds-api.com/v4/sports?apiKey={api_key}'.format(api_key=API_KEY)
...
def get_all_sports_data():
    ...
    for sport in sports:
        request_url = 'https://api.the-odds-api.com/v4/sports/{sport}/odds/?regions=us&oddsFormat=american&markets={market}&apiKey={api_key}'.format(sport=sport, api_key=API_KEY, market = MARKET)
        data = requests.get(request_url)
        ...
        obj['events'], obj['Total Arbitrage Opporutunities'] = arb_algo.detect_arbs_market(data, TOGGLE_LIVE)
        res.append(obj)
    return res
